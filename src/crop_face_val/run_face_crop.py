...
def build_detected_face_output_path(
    ...
    """
    Tạo path output cho face crop và giữ lại cấu trúc folder gốc.
    ...
    Output:
        data/detected_faces/Event_Public/n000002/abc_face01.jpg
    """

    absolute_source_path = resolve_project_path(source_image_path)

    # Lấy thư mục cha của ảnh gốc so với RAW_EVENTS_DIR.
    # Ví dụ:
    # RAW_EVENTS_DIR = data/val/Event_Public
    # source parent = data/val/Event_Public/n000002
    # relative_parent = n000002
    try:
        relative_parent = absolute_source_path.parent.relative_to(RAW_EVENTS_DIR)
    except ValueError:
        # Fallback nếu vì lý do nào đó source không nằm trong RAW_EVENTS_DIR
        # thì dùng tên folder cha trực tiếp, ví dụ n000002
    ...
    output_dir = DETECTED_FACES_DIR / relative_parent
...
def run_face_crop() -> None:
    ...
    #     encoding="utf-8-sig",
    # )
    # Không lưu report cho tập val, chỉ thống kê lỗi trong terminal
    failed_crop_df = pd.DataFrame(
        failed_crop_records,
        columns=[
            "face_id",
    ...
    print("Bỏ qua lưu report lỗi cho tập val.")
    print("=" * 70)
